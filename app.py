...
class Application:
    ...
    def __init__(self):
        # 直接在代码中设置参数
        self.reload_interval = 2700  # 45分钟（单位：秒）
        self.reload_scopes = [
            core_entities.LifecycleControlScope.PLATFORM.value,
            # core_entities.LifecycleControlScope.PLUGIN.value,  # 按需启用
            # core_entities.LifecycleControlScope.PROVIDER.value
        ]
        self._reload_lock = asyncio.Lock()  # 保留并发控制锁
    ...
    async def schedule_reload(self):
        ...
        while True:
            try:
                async with self._reload_lock:  # 保持锁机制
                    self.logger.info(f"定时重载将在 {self.reload_interval} 秒后执行...")
                    await asyncio.sleep(self.reload_interval)
                    
                    for scope in self.reload_scopes:
                        try:
                            start = asyncio.get_event_loop().time()
                            self.logger.info(f"[热重载] 开始执行 {scope} 重载")
                            await self.reload(scope)
                            cost = asyncio.get_event_loop().time() - start
                            self.logger.info(f"[热重载] {scope} 重载完成，耗时 {cost:.2f}s")
                        except Exception as e:
                            self.logger.error(f"[热重载] {scope} 重载失败: {str(e)}", exc_info=True)
                            await asyncio.sleep(5)  # 单个scope失败后缓冲
                            
            except asyncio.CancelledError:
                self.logger.warning("[热重载] 定时任务已被取消")
                break
            except Exception as e:
                self.logger.critical(f"[热重载] 定时器异常: {str(e)}", exc_info=True)
                await asyncio.sleep(60)  # 防止错误循环
